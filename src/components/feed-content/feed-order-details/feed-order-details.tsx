  ...
  return (
    <div>
      <header>
        ...
        <p className={`${styles.status} text text_type_main-default mt-3`}>{order && OrderStatus[order.status]}</p>
      </header>
      <p className='text text_type_main-medium mt-15'>Состав:</p>
      <ul className={`${styles.list} mt-6`}>
        {
          orderIngredients?.bun?._id && <li className={styles.item}>
            <IngredientImage
            ...
            />
            <p className={`${styles.name} text text_type_main-default`}>{orderIngredients?.bun.name}</p>
            <p className={`${styles.price} text text_type_digits-default`}>2 x {orderIngredients?.bun.price}&nbsp;<CurrencyIcon type='primary'/></p>
          </li>
        }
        {
          orderIngredients?.ingredientsArr.length && orderIngredients?.ingredientsArr.map(ingredient =>
            <li className={styles.item} key={nanoid()}>
              ...
              <p className={`${styles.price} text text_type_digits-default`}>1 x {ingredient.price}&nbsp;<CurrencyIcon type='primary'/></p>
            </li>
          )
        }
      </ul>
