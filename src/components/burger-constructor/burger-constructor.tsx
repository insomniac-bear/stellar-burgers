...
const BurgerConstructor: FC = () => {
  ...
  const { order, orderIngredientsId, orderIngredientsIdRequest } = useSelector(
    (store) => store.order
  );
    ...
    <section className={`${constructorStyles.container} pt-25 pl-4`}>
      <ul
        className={`${constructorStyles.list} ${
          isHover && constructorStyles.list_type_isHover
        }`}
        ref={dropTarget}
      >
        ...
        <li>
          {main && (
            <ul
              className={`${constructorStyles.mainingredientsList} mt-4 pr-4`}
            >
              {main.map((item, index) => (
