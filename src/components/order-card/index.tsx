...
const OrderCard: FC<IOrderCardProps> = ({
  ...
  const { bun, ingredientsArr } = intersection(
    ingredientIdList,
    ingredientsList
  );
  const date = formatDate(created);

  const showedIngredientsList =
  ...
  const price = ingredientsArr.reduce(
    (prevValue, item) => (prevValue += item.price),
    bunPrice
  );

  const unshowedIngredientsCount =
    ingredientsArr.length - MAX_SHOWED_INGREDIENTS > 0
      ? ingredientsArr.length - MAX_SHOWED_INGREDIENTS
      : 0;
  ...
  return (
    <li className={`${styles.item} pt-6 pr-6 pb-6 pl-6 mb-4`}>
      ...
      >
        <p className={`${styles.number} text text_type_main-default`}>
          #{number}
        </p>
        ...
        <ul className={`${styles.ingredients_list} mt-6`}>
          ...
          {!!showedIngredientsList.length &&
            showedIngredientsList.map((ingredient, index) => (
              <li
                key={nanoid()}
                className={styles.ingredients_item}
                style={{
